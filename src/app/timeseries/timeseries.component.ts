...
import { SosCapabilities, Timeseries, TimeseriesConfiguratorModalComponent } from './';
...
export class TimeseriesComponent implements OnInit {
  ...
  days = 20 * 365; // Days you want to subtract
  dateNow = new Date();
  dateFrom = new Date(this.dateNow.getTime() - (this.days * 24 * 60 * 60 * 1000));

  preconfiguredNgmpTs: Timeseries = <Timeseries>{
    sosUrl: 'https://ggw.gns.cri.nz/sos/service',
    offering: '1416',
    procedure: '1416',
    observedProperty: '1416',
    featureOfInterest: '49',
    fromDate: this.dateFrom,
    toDate: this.dateNow,
    timeseriesName: 'electrical conductivity @ NGMP Feature 49'
  };
   ...
   * @param idx
   */
  openTimeseriesConfigurator( event: any, idx: number ) {

    this.modal.showModal(this.timeseries[ idx ], ( ts ) => {
      // when null then editing was cancelled. Normally the callback does not get called but just to be sure.
      this.loadConfigureTimeSeries(ts, idx);
    });
  ...
  onAddNgmpExample(): void {
    let len = this.timeseries.length;
    this.loadConfigureTimeSeries(this.preconfiguredNgmpTs, len);
  }
      ...
      this.loadTimeseriesData(ts).subscribe(
        ( timeseries ) => {
          ...
          if (idx < 0) {
            ...
            this.timeseries[ idx ] = timeseries;
            Plotly.purge('plotly');
