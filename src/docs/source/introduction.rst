...
1. Search, discover, explore, download research outputs

2. Presentation and Visualisation of research outputs

3. Publish research output

Scenario: Decision maker wants an overview of information on groundwater resource management
...
- #ScienceDomainSearch: THE developed CATEGORIES to guide users through the web interface to list datasets,
- models, reports and case studies as cards for the category search criteria
- #CSWIndexing: in order to provide the fine-grained retrieval of records based on the categories
- #AccessCheckTracking

Scenario: Researcher wants to find groundwater-related information and download datasets
...
Related Use Case Packages:
- #ScienceDomainSearch: THE CATEGORIES
- #CSWIndexing
- #ExpertSearch: with spatial and temporal constraints (map search with inline results) and manual choice of query string/keyword search
- #AccessCheckTracking

Scenario: Consultant wants to find groundwater-related information and download research
...
Scenario: Full info and links for result items
...
Scenario: Show a Single Dataset or a Case Study on a map
...
Scenario: plot a Single (or multiple related/comparable) presumably time-series datasets as graphs
...
Scenario: Visualise a 3D model/scene
...
Scenario: Researcher or science staff wants to publish research outputs (a "dataset")
...
Related Use Case Packages:
- #UserAuth:  because they need to login to edit/upload data
- #AddEditMetadataRecord: add/edit metadata (MD_Metadata)
- #UploadHandleFile: upload/store/delete a file
- #OWCCollections: store the references to metadata records and files in own collection list,
to track ownership and subsequently enable to edit/delete 'own' data

Scenario: Researcher or science staff wants to publish research outputs (a "report")

A researcher wants to publish a science oder consultancy report, best practice white paper,policy advice article,
one pager/flyer... They will upload their data ... They will be supported to provide ISO metadata
...
Related Use Case Packages:
- #UserAuth
- #AddEditMetadataRecord: Type 'dataset', or 'nonGeographicDataset' (e.g. purely text report)
- #UploadHandleFile
- #OWCCollections

Scenario: Researcher or science staff wants to publish research outputs (a "model")
...
Related Use Case Packages:
- #UserAuth
- #AddEditMetadataRecord: Type 'dataset' (3D layer files), 'model', or 'software', 'nonGeographicDataset' (e.g. purely numerical)
- #UploadHandleFile
- #OWCCollections

Scenario: Researcher or science staff wants to publish research outputs (a "case study")
...
Related Use Case Packages:
- #UserAuth
- #AddEditMetadataRecord: to provide a distinct MD_Metadata record for the case study as a collection of data 'series'
- #UploadHandleFile
- #OWCCollections: to create, edit and 'publish specifically crafted collections as case studies

Scenario: Lead researcher, science staff or manager wants to control who adds data to his case studies
...
(initially at least). All users can then "apply" on projects, organisations, case studies to be added,
which needs to be approved by the owning power users (maybe just the owning user actually)

Related Use Case Packages:
- #UserAuth
- #OWCCollections
- #DataUserAdmin: uploaded data belongs always to the user who uploaded and only they can edit/update/delete their
...
Scenario: A data owner/publisher wants to know about the usage of his data
...
- #OWCCollections
- #DataUserAdmin:
- #AccessCheckTracking: When data is searched for, results presented/requested as cards or on map search,
